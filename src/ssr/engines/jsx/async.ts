...
export async function resolveAsync(element: JSXElement): Promise<string> {
  ...
  if (Array.isArray(element)) {
    const resolved = await Promise.all(element.map((el: JSXElement) => resolveAsync(el)))
 ...
 * ])
 */
export async function renderParallel(components: JSXElement[]): Promise<string> {
 ...
 * @returns Promise<string> - All components rendered and joined
 */
export async function renderSequential(components: JSXElement[]): Promise<string> {
