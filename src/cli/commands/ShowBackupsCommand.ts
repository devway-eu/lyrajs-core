...
export class ShowBackupsCommand {
  ...
  async execute() {
    ...
    try {
      ...
      const rows: BackupRow[] = backups.map(backup => ({
        file: backup.file,
      ...
      // Build table header
      output.push(
        `┌${"─".repeat(fileWidth + 2)}┬${"─".repeat(sizeWidth + 2)}┬${"─".repeat(createdWidth + 2)}┐`
      )
      output.push(
        `│ ${"FILE".padEnd(fileWidth)} │ ${"SIZE".padEnd(sizeWidth)} │ ${"CREATED".padEnd(createdWidth)} │`
      )
      output.push(
        `├${"─".repeat(fileWidth + 2)}┼${"─".repeat(sizeWidth + 2)}┼${"─".repeat(createdWidth + 2)}┤`
      )
      ...
      for (const row of rows) {
        output.push(
          `│ ${row.file.padEnd(fileWidth)} │ ${row.size.padEnd(sizeWidth)} │ ${row.created.padEnd(createdWidth)} │`
        )
      }
