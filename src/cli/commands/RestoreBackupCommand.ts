  ...
  async execute(args: string[]) {
    ...
    if (!migrationVersion) {
      LyraConsole.error(
        ...
        "Usage: npx maestro restore:backup <MIGRATION_VERSION>"
      )
      return
    }
    ...
    try {
      ...
      if (rows.length === 0) {
        LyraConsole.error(
          `Migration version ${migrationVersion} not found`,
          "Run 'npx maestro show:migrations' to see available migrations"
        )
        return
      }

      const backupPath = (rows[0] as any).backup_path

      if (!backupPath) {
        LyraConsole.error(
          `No backup found for migration ${migrationVersion}`,
          "This migration did not create a backup (non-destructive migration)"
        )
        return
      }
      ...
      LyraConsole.warn(
        ...
        "All data changes since this backup will be LOST",
        "",
      ...
      LyraConsole.success(
        ...
        `Restored from migration: ${migrationVersion}`,
        "You may need to run migrations again to get back to the desired state"
      )
    } catch (error: any) {
