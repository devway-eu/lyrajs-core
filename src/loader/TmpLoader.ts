...
import {__lyraRootDir, __projectRootDir} from "@/core/index";
...
export class TmpLoaderClass {
    ...
    private async cleanTmpFolder(folder: string) {
        const full = join(__lyraRootDir, folder)
        if (existsSync(full)) await rm(full, { recursive: true, force: true })
        await mkdir(full, { recursive: true })
    }

    private async compileFileToTmp(source: string, outDir: string) {
    ...
    private async refreshDir(sourceSubdir: string, tmpSubdir: string) {
