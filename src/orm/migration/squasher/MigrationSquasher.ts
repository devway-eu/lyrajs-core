...
import { MigrationRecord, TableDefinition } from "../interfaces/types"
...
export class MigrationSquasher {
   ...
   */
  async squash(targetVersion?: string): Promise<void> {
    ...
    const migrationsToSquash = migrations.filter(
      (m: MigrationRecord) => m.version <= target && !m.squashed
    )
    ...
    LyraConsole.info(
      `\u2699 Squashing ${migrationsToSquash.length} migrations up to version ${target}...`
    )
    ...
    const migrationContent = this.buildSquashedMigration(
      className,
      timestamp,
      currentSchema.getTables(),
      migrationsToSquash
    )
    ...
    const versions = migrationsToSquash.map((m: MigrationRecord) => m.version)
    await this.connection.query(
      `UPDATE migrations SET squashed = TRUE WHERE version IN (${versions.map(() => "?").join(",")})`,
      versions
    )

    LyraConsole.success(
      "\u2713 Migration squashing completed",
      `Squashed migration: ${filename}`,
      `Migrations squashed: ${migrationsToSquash.length}`,
      `Version range: ${migrationsToSquash[0].version} to ${target}`
    )
  }
  ...
  private buildSquashedMigration(
    ...
    squashedMigrations: MigrationRecord[]
  ): string {
    let content = ""
   ...
   */
  private generateSquashedUpSQL(tables: TableDefinition[]): string {
    let sql = ""
   ...
   */
  private generateSquashedDownSQL(tables: TableDefinition[]): string {
    let sql = ""
   ...
   */
  private generateSquashedDryRunSQL(tables: TableDefinition[]): string {
    let sql = ""
   ...
   */
  private generateCreateForeignKeySQL(tableName: string, fk: any): string {
