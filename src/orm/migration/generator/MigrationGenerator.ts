...
export class MigrationGenerator {
   ...
   */
  private async promptForRenameConfirmation(diff: SchemaDiff): Promise<void> {
    const CYAN = '\x1b[36m'
    ...
    const confirmedColumnRenames: ColumnRename[] = []
    const confirmedTableRenames: TableRename[] = []
    ...
    const rejectedColumnRenames = diff.columnsToRename.filter(
      r => !confirmedColumnRenames.find(c => c.table === r.table && c.from === r.from)
    )
   ...
   */
  private generateRenameColumnSQL(rename: ColumnRename): string {
