import { DatabaseSchema, SchemaDiffResult } from '../interfaces/DatabaseSchema'
import {
  TableDefinition,
  ColumnDefinition,
  SchemaDiff,
...
export class SchemaDiffer {
   ...
   */
  private findNewTables(current: DatabaseSchema, desired: DatabaseSchema): TableDefinition[] {
   ...
   */
  private getCommonTables(current: DatabaseSchema, desired: DatabaseSchema): string[] {
    const currentNames = new Set(current.getTableNames())
   ...
   */
  private findNewColumns(currentTable: TableDefinition, desiredTable: TableDefinition): ColumnDefinition[] {
   ...
   */
  private findRemovedColumns(currentTable: TableDefinition, desiredTable: TableDefinition): string[] {
  ...
  private findModifiedColumns(
    ...
    desiredTable: TableDefinition
  ): Omit<ColumnChange, 'table'>[] {
    const changes: Omit<ColumnChange, 'table'>[] = []
   ...
   * Find new indexes
   */
  private findNewIndexes(currentTable: TableDefinition, desiredTable: TableDefinition) {
   ...
   */
  private findNewForeignKeys(currentTable: TableDefinition, desiredTable: TableDefinition) {
