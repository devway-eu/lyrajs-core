...
export class RenameDetector {
  private readonly SIMILARITY_THRESHOLD = 0.6 // 60% similarity minimum
  private readonly HIGH_CONFIDENCE_THRESHOLD = 0.8 // 80% for high confidence
  ...
  detectColumnRenames(
    ...
    const uniqueRenames: ColumnRename[] = []
  ...
  detectTableRenames(
    ...
    for (const removed of removedTables) {
      let bestMatch: { table: TableDefinition; confidence: number } | null = null
  ...
  private areTypesCompatible(type1: string, type2: string): boolean {
    ...
    if (textTypes.includes(t1) && textTypes.includes(t2)) return true
    if (floatTypes.includes(t1) && floatTypes.includes(t2)) return true
   ...
   */
  private doConstraintsMatch(col1: ColumnDefinition, col2: ColumnDefinition): boolean {
    return (
